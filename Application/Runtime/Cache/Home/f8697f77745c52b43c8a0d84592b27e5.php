...
<div class="sign-document-class">
    <a href="<?php echo U('sendsign/alllist');?>">全部</a>
    <a href="<?php echo U('sendsign/mysignlist');?>" >我发起</a>
    <a href="<?php echo U('sendsign/waitmysignlist');?>" class="sign-document-class-cur">我签署</a>
</div>
        ...
        layui.use('flow', function () {
            ...
            flow.load({
                ...
                , done: function (page, next) { //到达临界点（默认滚动触发），触发下一页
                    ...
                    $.get('/home/sendsign/getwaitmysignlist?page=' + page, function (res) {
                        if(res.code ==2){
                        ...
                        layui.each(res.data, function (index, item) {
                            if(item.status == 1){
                                var statusname = '<span class="sign-progress">签署中</span>';
                            }else if(item.status == 2){
                                var statusname = '<span class="sign-complete">已完成</span>';
                            }else if(item.status == 3){
                                var statusname = '<span class="sign-refusal">已拒签</span>';
                            }else if(item.status == 4){
                                var statusname = '<span class="past-due">已过期</span>';
                            }
                            lis.push('<li>' +
                                '        <a  href="/home/sendsign/forminfo/id/'+item.id+'">' +
                                '            <div class="sign-document-time">'+item.leftime+'</div>' +
                                '            <div class="sign-document-cont">' +
                                '                <p class="sign-document-title">'+item.name+'</p>' +
                                '                <p class="sign-document-tips">' +
                                statusname +
                                '                </p>' +
                                '                <p class="sign-document-date">' +
                                '                    <span>发起时间：'+item.start_time+'</span>' +
