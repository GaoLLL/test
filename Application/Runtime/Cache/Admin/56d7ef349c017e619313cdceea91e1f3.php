...
<div>
    ...
    <form action="/index.php/Admin/Bullhorn/index">
        ...
        <!--<span class="search_list_content">
            &nbsp;&nbsp;商品类型：
            <select name="pid" width="50" class="search_input">
                <option value="0" <?php if(($search["pid"]) == "0"): ?>selected<?php endif; ?> >请选择</option>
                <option value="1" <?php if(($search["pid"]) == "1"): ?>selected<?php endif; ?>>课程商品</option>
                <option value="2" <?php if(($search["pid"]) == "2"): ?>selected<?php endif; ?>>兑换商品</option>
                <option value="3" <?php if(($search["pid"]) == "3"): ?>selected<?php endif; ?>>返现金额</option>
            </select>
        </span>-->
            <span class="search_list_content">
            ...
            <div class="mt-20">
                    ...
                    <tbody>
                        <?php if(empty($list)): ?><tr class="text-c va-m">
                            ...
                            <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c">
                                    <td width="40"><input name="id[]" type="checkbox" value="<?php echo ($vo["id"]); ?>"></td>
                                    <td width="40"><span class="label label-success radius"><?php echo ($k); ?></span></td>
                                    <td class="text-l" width="150" style="text-align: center!important;"><?php echo ($vo["title"]); ?></td>
                                    <td class="text-l" style="text-align: center!important;"><?php echo (date('Y-m-d H:i:s',$vo["addtime"])); ?></td>
                                    <td class="td-manage">
                ...
                <div class="scott">
                    ...
                    <p style="margin-bottom:15px;">
                        共<?php echo ($count); ?>条记录/每页显示<input type="text" size="5" name="pagesize"
                                                value="<?php echo ($pagesize); ?>"
                                                style="text-align:center;" />条&nbsp;<input type="submit" value="GO" />
                    </p><?php echo ($page); ?>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function () {
        ...
        $('.add').click(function () {
            var index = layer.open({
                ...
                area: ['700px', '470px'],
                fix: false, //不固定
                content: '/index.php/Admin/Bullhorn/add',
                btn: ['确定', '关闭'],
                maxmin: true,
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Bullhorn/add', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.save').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['700px', '470px'],
                fix: false, //不固定
                content: '/index.php/Admin/Bullhorn/save/id/' + id,
                btn: ['确定', '关闭'],
                maxmin: true,
                yes: function () {
                    var form = layer.getChildFrame('form', index);
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Bullhorn/save', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.del').click(function () {
            ...
            var index = layer.msg('是否删除' + title + '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Bullhorn/del', {id: id}, function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
        ...
        $('.alldel').click(function () {
            var index = layer.msg('是否确认删除' +  '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Bullhorn/alldel', $('form').serialize() , function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 1000, end: function () {
                                    window.location.reload();
...
<script>
    $('.rbac').click(function () {
        ...
        layer.open({
            ...
            title: '管理员授权管理',
            area: ['100vw', '100vh'],
            content: '/index.php/Admin/Bullhorn/rbac/id/' + id + '.html'
        })
    });
</script>