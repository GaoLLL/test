...
<div>
    <nav class="breadcrumb">
        <i class="fa fa-info-circle"></i> 全局菜错误代码表
    ...
    <form action="/index.php/Admin/Setting/index">
        <div class="search_list">
        <span class="search_list_content">
            错误代码：
            <input type="text" name="error_id" value="<?php echo ($search['error_id']); ?>" width="50" class="search_input"/>
        </span>
        <span class="search_list_content">
                ...
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <?php if(empty($list)): ?><tr class="text-c va-m">
                        ...
                        <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c">
                                ...
                                <td class="text-l"><?php echo ($vo["error_id"]); ?></td>
                                <td class="text-l"><?php echo ($vo["error_detail"]); ?></td>
                                <td class="td-manage">
        ...
        $('.alldel').click(function () {
            var index = layer.msg('是否确认删除' +  '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Setting/alldel', $('form').serialize() , function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 1000, end: function () {
                                    window.location.reload();
