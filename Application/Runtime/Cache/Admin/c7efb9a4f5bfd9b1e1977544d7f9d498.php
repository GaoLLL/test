        ...
        <tr>
            ...
            <td align="left">
                <select class="input-text radius" style="width:200px;height: 30px" name="pid">
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($info["pid"]) == $vo["id"]): ?>selected<?php endif; ?>  ><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </td>
        </tr>
        <tr>
            <td align="right" height="50" width="50%"><label>文件名称：</label></td>
            <td align="left"><input name="title" value="<?php echo ($info["title"]); ?>" class="input-text radius" placeholder="输入文件名称" style="width:200px;height: 30px"></td>
        </tr>
        <tr>
            <td align="right" height="50"><label>上传路径：</label></td>
            <td align="left">
                <input type="hidden" name="img" id="imgs" value="<?php echo ($info["img"]); ?>"/>
                <!--<a id="show" href="" target="_blank">预览</a>-->
                <input type="text" value="<?php echo ($info["img"]); ?>" disabled id="shownum"/>
