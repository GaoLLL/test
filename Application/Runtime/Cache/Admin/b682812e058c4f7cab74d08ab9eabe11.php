...
<div>
    ...
    <form action="/index.php/Admin/Admin/index">
        <div class="search_list">
        <span class="search_list_content">
            用户名：
            <input type="text" name="username" value="<?php echo ($search['username']); ?>" width="50" class="search_input"/>
        </span>
        <span class="search_list_content">
                ...
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <?php if(empty($list)): ?><tr class="text-c va-m">
                        ...
                        <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="text-c">
                                ...
                                <td class="text-l"><?php echo ($vo["group"]); ?></td>
                                <td class="text-l"><?php echo ($vo["username"]); ?></td>
                                <td class="text-l"><?php echo ($vo["password"]); ?>(加密)</td>
                                <td class="text-l">
                                    <?php if(($vo["login_time"]) == "0"): ?>尚未登陆
                                        <?php else: ?>
                                        <?php echo (date('Y-m-d H:i:s',$vo["login_time"])); endif; ?>
                                </td>
                                <td class="text-l">
                                    <?php if(($vo["login_ip"]) == ""): ?>尚未登陆
                                        <?php else: ?>
                                        <?php echo ($vo["login_ip"]); endif; ?>
                                </td>
                                <td class="td-manage">
    ...
    $(document).ready(function () {
        ...
        $('.add').click(function () {
            var index = layer.open({
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Admin/add', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.save').click(function () {
            ...
            var index = layer.open({
                ...
                area: ['700px', '470px'],
                fix: false, //不固定
                content: '/index.php/Admin/Admin/save/id/' + id,
                btn: ['确定', '关闭'],
                maxmin: true,
                yes: function () {
                    var form = layer.getChildFrame('form', index);
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Admin/save', form.serialize(), function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 2000,
                                end: function () {
        ...
        $('.del').click(function () {
            ...
            var index = layer.msg('是否删除' + title + '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Admin/del', {id: id}, function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
        ...
        $('.alldel').click(function () {
            var index = layer.msg('是否确认删除' +  '? <br>删除后不可恢复，请谨慎操作！', {
                ...
                yes: function () {
                    ...
                    flag = 2;
                    $.post('/index.php/Admin/Admin/alldel', $('form').serialize() , function (data) {
                        if (data.status == 1) {
                            layer.msg(data.msg, {
                                time: 1000, end: function () {
                                    window.location.reload();
...
<script>
    $('.rbac').click(function () {
        ...
        layer.open({
            ...
            title: '管理员授权管理',
            area: ['100vw', '100vh'],
            content: '/index.php/Admin/Admin/rbac/id/' + id + '.html'
        })
    });
</script>